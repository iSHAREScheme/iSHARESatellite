...
function isChaincodeAlreadyCommittedAtSequence() {
local CH_NAME=$1
local CC_NAME=$2
local CC_SEQUENCE=$3
...
committed_output=$(peer lifecycle chaincode querycommitted --channelID "${CH_NAME}" --name "${CC_NAME}" 2>&1)
committed_res=$?
printf "%s\n" "${committed_output}"

if [ $committed_res -ne 0 ]; then
  return 1
fi

committed_seq=$(printf "%s" "${committed_output}" | sed -n 's/.*Sequence: \([0-9][0-9]*\).*/\1/p' | head -n 1)
if [[ -z "${committed_seq}" ]]; then
  return 1
fi

if [[ "${committed_seq}" == "${CC_SEQUENCE}" ]]; then
  return 0
fi
...
if isChaincodeAlreadyCommittedAtSequence "${CH_NAME}" "${CC_NAME}" "${CC_SEQUENCE}"; then
   warnln "Chaincode definition is already committed at sequence ${CC_SEQUENCE}; skipping approve for this org."
   return 0
fi
...
set -x
peer lifecycle chaincode approveformyorg -o $ORDERER_ENDPOINT --tls --cafile $ORDERER_TLS_CA_FILE "${ORDERER_TLS_HOSTNAME_ARGS[@]}" --channelID $CH_NAME --name $CC_NAME --version ${CHAINCODE_VERSION} --package-id $CC_PACKAGE_ID --sequence $CC_SEQUENCE --signature-policy "${CC_POLICY}" --waitForEvent
res=$?
set +x
if [ $res -ne 0 ]; then
   if isChaincodeAlreadyApproved "${CH_NAME}" "${CC_NAME}" "${CC_SEQUENCE}"; then
   ...
   if isChaincodeAlreadyCommittedAtSequence "${CH_NAME}" "${CC_NAME}" "${CC_SEQUENCE}"; then
      warnln "Chaincode definition was committed while approve call failed; continuing."
      return 0
   fi
